...
int ContractCommonInterface::GenVin(const std::string& fromAddr, const uint64_t& expend, const uint64_t& tempGas, CTxUtxo * txUtxo, std::vector<std::string>& utxoHashs, uint64_t& total, bool isFindUtxo, bool isSign, const std::string& sender)
{
    // Find utxo
    std::multiset<TxHelper::Utxo, TxHelper::UtxoCompare> setOutUtxos;
    auto ret = TxHelper::FindUtxo(fromAddr, TxHelper::kMaxVinSize, expend, total, tempGas, setOutUtxos, isFindUtxo,sender);
    if (ret != 0)
    {
...
int Evmone::VerifyUtxo(const std::string& sender, const CTransaction& tx, const CTransaction& callOutTx)
{
    ...
    for(const auto& vout : tx.utxo().vout())
    {
        ...
        if(global::ca::kVirtualBurnGasAddr == vout.addr())
        {
            txBurnGas = vout.value();
        }
    }

    uint64_t tagetBurn = 0, tagetBalance = 0;
    uint64_t callOutTxVoutValue = 0 , callOutTxBurnGas = 0;
    for(const auto& vout : callOutTx.utxo().vout())
    {
        callOutTxVoutValue += vout.value();
        if(vout.addr() == global::ca::kVirtualDeployContractAddr || vout.addr() == global::ca::kVirtualCallContractAddr){
            tagetBurn = vout.value();
            continue;
        }

        if(sender == vout.addr()){
            tagetBalance = vout.value();
            continue;
        }

        if(global::ca::kVirtualBurnGasAddr == vout.addr())
        {
            callOutTxBurnGas = vout.value();
        }

    }

    if(tagetBurn > 2 * srcBurn){
        ERRORLOG("tagetBurn{}, srcBurn:{}", tagetBurn, srcBurn);
        return -3;
    }
    
    uint64_t srcTotalAmount = srcBurn + srcBalance;
    uint64_t targetTotalAmount = tagetBurn + tagetBalance;
    if(srcTotalAmount != targetTotalAmount){
        ERRORLOG("srcBurn: {} , tagetBurn:{} ", srcBurn, tagetBurn);
        ERRORLOG("srcBalance: {} , tagetBalance:{} ", srcBalance, tagetBalance);
        ERRORLOG("srcTotalAmount : {}, targetTotalAmount : {}", srcTotalAmount, targetTotalAmount);
        return -4;
    }

    if(txBurnGas != callOutTxBurnGas)
    {
        ERRORLOG("The gas value does not match! txBurnGas : {}, callOutTxBurnGas : {}",txBurnGas, callOutTxBurnGas);
        return -5;
    }
    
    if(txVoutValue != callOutTxVoutValue)
    {
        ERRORLOG("The vout value does not match! txVoutValue : {}, callOutTxVoutValue : {}",txVoutValue , callOutTxVoutValue);
        return -6;
    }
...
//    DON::DonWasmtimeVM vm(hostFunctions.get(), code_str);
...
//        return retVal;
//    }
//    auto storage = MagicSingleton<DON::WasmStore>::GetInstance();
//    storage->CreateTrie(rootHash, contractAddress);
//    MagicSingleton<DON::StoreManager>::GetInstance()->InsertStore(contractAddress, storage);
//    MagicSingleton<DON::StoreManager>::GetInstance()->setCurrentContractAddr(contractAddress);
//
//    std::shared_ptr<DON::WasmtimeVMhost> Actuator =  MagicSingleton<DON::WasmtimeVMhost>::GetInstance();
//    DON::DonWasmtimeVM vm(Actuator.get(), strCode);
//
//    uint64_t balance = 0;
//    GetBalanceByUtxo(fromAddr, balance);
//
...
//    auto tansationTargets = MagicSingleton<DON::StoreManager>::GetInstance()->GetTrasationTarget(contractAddr);
//
//    std::map<std::string,std::map<std::string,uint64_t>> transfersMap;
...
//{
//    std::string contractAddr = MagicSingleton<DON::StoreManager>::GetInstance()->getCurrentContractAddr();
//    auto tansationTarges = MagicSingleton<DON::StoreManager>::GetInstance()->GetTrasationTarget(contractAddr);
//
//    std::map<std::string,std::map<std::string,uint64_t>> transfersMap;
...
//    std::map<std::string,std::string> items;
//
//    auto it = MagicSingleton<DON::StoreManager>::GetInstance();
...
//    jTxInfo[Evmone::contractPreHashKeyName] = items;
//
//    auto destruction_data = MagicSingleton<DON::StoreManager>::GetInstance()->destruction_data;
//    std::map<std::string,std::string> destructItems;
...
//{
//    auto contractStores = MagicSingleton<DON::StoreManager>::GetInstance()->GetAllStoreMap();
...
{ 
    ...
    uint64_t tempGas = 0;
    if(TestGenerateGas(outTx, VoutSize, tempGas) != 0)
	{
		ERRORLOG(" gas = 0 !");
		return -9;
	}
    ...
    for(const auto& iter : transfersMap)
    {
        ...
        if(iter.first != fromAddr){
            auto ret = ContractCommonInterface::GenVin(iter.first, allBalance, 0, outTx.mutable_utxo(), utxoHashs, total, isFindUtxo, isSign,fromAddr);
            if(ret < 0)
            {
            ...
            auto ret = ContractCommonInterface::GenVin(iter.first, Allexpend, tempGas, outTx.mutable_utxo(), utxoHashs, total, isFindUtxo, isSign);
            if(ret < 0)
            {
                ERRORLOG("GenVin fail!!! ret:{}",ret);
                return -2;
    ...
    if(found == fromBalance.end())
    {
        ...
        auto ret = ContractCommonInterface::GenVin(fromAddr, expend, tempGas, outTx.mutable_utxo(), utxoHashs, total, isFindUtxo, isGenSign);
        if(ret < 0)
        {
            ERRORLOG("GenVin fail!!! ret:{}",ret);
            return -2;
