...
uint64_t TxHelper::calculateUtxoValue(const std::string& balance)
{
	std::string underline = "_";
						...
						bool isFindUtxo)
{
	...
	for (const auto& addr : fromAddr)
	{
		...
		for (const auto& hash : vecUtxoHashs)
		{
			...
			utxo.value = calculateUtxoValue(balance);	
...
int TxHelper::FindUtxo(const std::string& addr,
						...
						bool isFindUtxo,const std::string& sender)
{
	...
	boost::circular_buffer<TxHelper::Utxo> circularBuffer(Utxos.size());
	for (const auto& utxo : Utxos) {
		DEBUGLOG("utxo addr: {}  hash :{}, utxo value: {}", utxo.addr, utxo.hash, utxo.value);
        circularBuffer.push_back(utxo);
	}

    size_t seed;
	if(sender.empty())
	{
		DEBUGLOG("FindUtxo radom seed string addr : {}", addr);
	    std::hash<std::string> hasher;
    	seed = hasher(addr);	
	}else{
		DEBUGLOG("FindUtxo radom seed string sender : {}", sender);
	    std::hash<std::string> hasher;
    	seed = hasher(sender);	
	}

    std::default_random_engine generator(seed);
    std::uniform_int_distribution<int> distribution(0, Utxos.size() - 1);
   	int randomDigit = distribution(generator);
	DEBUGLOG("FindUtxo randomDigit: {}", randomDigit);

	auto startPosition =  circularBuffer.begin() + randomDigit;

	int i = 1;
	auto it = startPosition;
	uint64_t amountSum = 0;

	uint64_t Gas = (tempGas + (1 * 64) + 34) * 100;
	uint64_t tempExpend = expend + Gas;

	do {
		amountSum += it->value;
		DEBUGLOG("FindUtxo add setOutUtxos hash: {}, value: {}", it->hash, it->value);
		setOutUtxos.insert(*it);
		
		Gas = (tempGas + (i * 64) + 34) * 100;
		tempExpend = expend + Gas;

		++i;
		++it;

		if (it == circularBuffer.end()) {
			it = circularBuffer.begin();
		}

		if (amountSum >= tempExpend) {
			break;
		}
	} while (it != startPosition);
									...
									Vrf & info, bool isFindUtxo)
{
	...
	targetAddrs.insert(make_pair(*fromAddr.rbegin(), 0));
										...
										bool isFindUtxo)
{
	...
	toAddr.insert(std::make_pair(global::ca::kVirtualBurnGasAddr, 0));
										...
										,TxHelper::vrfAgentType &type ,Vrf & information, bool isFindUtxo)
{
	...
	toAddrs.insert(std::make_pair(fromAddr, 0));
	toAddrs.insert(std::make_pair(global::ca::kVirtualBurnGasAddr, 0));
										...
										,TxHelper::vrfAgentType &type ,Vrf & information, bool isFindUtxo)
{
	...
	targetAddrs.insert(std::make_pair(fromAddr, 0 ));
	targetAddrs.insert(std::make_pair(global::ca::kVirtualBurnGasAddr, 0));
										...
										Vrf & information, bool isFindUtxo)
{
	...
	toAddrs.insert(std::make_pair(global::ca::kVirtualStakeAddr, 0));
	toAddrs.insert(std::make_pair(global::ca::kVirtualBurnGasAddr, 0));
