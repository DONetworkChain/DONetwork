...
int CheckBlocks::_ToCheck()
{
    ...
    while(true)
    {
        ...
        ret = MagicSingleton<SyncBlock>::GetInstance()->_GetSyncNode(UINT32_MAX, _topBlockHeight + 1000, pledgeAddr, sendNodeIds);
        if(ret != 0)
        {
...
int CheckBlocks::DoNewSync(std::vector<std::string> sendNodeIds, std::vector<std::string>& pledgeAddr, uint64_t selfNodeHeight)
{
    ...
    for(const auto& sync_heiht: needSyncHeights)
    {
        ...
        ret = MagicSingleton<SyncBlock>::GetInstance()->_RunNewSyncOnce(pledgeAddr, chain_height, selfNodeHeight, sync_heiht - 100, sync_heiht, UINT32_MAX);
        sleep(10); 
        if(ret != 0)
        {
