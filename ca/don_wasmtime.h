#ifndef __DON_WASMTIME__
#define __DON_WASMTIME__
...
class DonWasmtimeVM {
  public:
    DonWasmtimeVM(WasmtimeVMhost *hostCaller, const std::string &bytes)
  ...
  private:
    wasmtime::_Result<std::string> _WasmtimeInit(int64_t gas, const std::string &wasmbytes,
        ...
        linker
            .func_wrap("env", "don_result_size",
                       [&]() -> int32_t { return _result.size(); })
            .ok();

        linker
            .func_wrap("env", "don_result",
                       [&](int32_t baseAddr) {
        ...
        linker
            .func_wrap("env", "don_set_run_retvalue",
                       [&](int32_t baseAddr, int32_t lenght) {
                           std::string ret;
                           char p = cum->data(store)[baseAddr];
                           for (int i = 0; i < lenght; i++) {
        ...
        linker
            .func_wrap("env", "don_input_param_size",
                       [&](int index) -> int32_t {
...
template <typename RetType, typename... ParamsType> class MakeHost {
  public:
    MakeHost(DON::WasmtimeVMhost &Call, const std::string &name,
             const std::function<RetType(ParamsType...)> &bc) {
        DON::WASMTIMECaller<RetType, ParamsType...> *host =new DON::WASMTIMECaller<RetType, ParamsType...>(bc);
