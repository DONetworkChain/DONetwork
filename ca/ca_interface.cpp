...
int GetBalanceReqImpl(const std::shared_ptr<GetBalanceReq>& req, GetBalanceAck & ack)
{
	...
	if(DBStatus::DB_SUCCESS != db_reader.GetUtxoHashsByAddress(addr, addr_utxo_hashs))
	{
		return -3;
	}
	...
	for (const auto &utxo_hash : addr_utxo_hashs)
	{
		...
		if (DBStatus::DB_SUCCESS != db_reader.GetUtxoValueByUtxoHashs(utxo_hash, addr, address_balance))
		{
			return -4;
		}
...
std::map<int32_t, std::string> GetReqCode()
{
	std::map<int32_t, std::string> errInfo = {  std::make_pair(0, "Get  List Success"), 
												std::make_pair(-1, "CreateWait failed"), 
												std::make_pair(-2, "time out"), 
												};
	return errInfo;												
}

std::map<int32_t, std::string> GetOnChainReqCode()
{
	std::map<int32_t, std::string> errInfo = {  std::make_pair(0, "Get  List Success"), 
												std::make_pair(-1, "CreateWait failed"), 
												std::make_pair(-2, "time out"), 
...
int SendCheckTxReq(const std::shared_ptr<IsOnChainReq>& msg,  IsOnChainAck & ack)
{
    ...
    std::string msg_id;
    std::map<std::string, int> successHash;
    ...
    CheckTxAck copyAck;
    std::multimap<std::string, uint32_t> tx_flag_hashs;
    ...
    for(auto & success : successHash)
    {
        ...
        DEBUGLOG("-------------->rate={}, send_num = {}",rate,send_num);
        alsuc->set_rate(rate);
...
int HandleIsOnChainReq(const std::shared_ptr<IsOnChainReq>& req, const MsgData & msgdata)
{
    ...
    auto errInfo = GetOnChainReqCode();
    IsOnChainAck ack;
    int ret = 0;
...
int GetRestInvestAmountReqImpl(const std::shared_ptr<GetRestInvestAmountReq>& msg,  GetRestInvestAmountAck & ack)
{
    ...
    invest_amount = global::ca::kMaxInvertAmt - sum_invest_amount;
