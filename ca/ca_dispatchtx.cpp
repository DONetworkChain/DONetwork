...
{   
    //Determines whether the distributed data is empty
    ...
    while (true)
    {
        ...
        */
        //If the current time is more than 15s find a distribution contract transaction
        if (timeValue >= timeBaseline + _contractWaitingTime)
        {
            ...
            std::cout << "before send tx to packager";
...
void ContractDispatcher::AddContractMsgReq(const std::string& contractHash, const ContractTxMsgReq &msg)
{
    ...
    ContractTempTxMsgReq  txmsg = msg.txmsgreq();
...
int ContractDispatcher::SendTxInfoToPackager(const std::string &packager, const NewVrf &info, std::vector<ContractTempTxMsgReq> &txsMsg, const std::set<std::string> nodeList)
{
    DEBUGLOG("***** SendTxInfoToPackager *****");
    ContractPackagerMsg msg;
    msg.set_version(global::kVersion);

    NewVrf * vrfInfo = msg.mutable_vrfinfo();
    vrfInfo->CopyFrom(info);
    

    for(auto & msgReq : txsMsg)
    {
        ContractTempTxMsgReq * _Msg = msg.add_txmsgreq();
    ...
    if(owner == packager)
    {
        ...
        MsgData msgdata;
        DEBUGLOG("HandlePackagerMsg");
        HandleContractPackagerMsg(sendTxmsg,msgdata);
        return 0;
    }
    
	net_send_message<ContractPackagerMsg>(packager, msg, net_com::Compress::kCompress_True, net_com::Encrypt::kEncrypt_False, net_com::Priority::kPriority_High_1);
    return 0;
}

std::vector<std::vector<ContractTempTxMsgReq>> ContractDispatcher::GetDependentData()
{
    ...
    std::vector<std::vector<ContractTempTxMsgReq>> groupedTxMsg;
    for(const auto & hashContainer : groupedDependencies)
    {
        std::vector<ContractTempTxMsgReq> messageVec;
...
std::vector<std::vector<ContractTempTxMsgReq>> ContractDispatcher::GroupDependentData(const std::vector<std::vector<ContractTempTxMsgReq>> & txMsgVec)
{
    //Protocol to group
    int size = txMsgVec.size();
    int parts = 8;

    std::vector<std::vector<ContractTempTxMsgReq>> groupTxMsg;
    if(size <= parts)
    {
    ...
    else
    {
        ...
        for(int i = 0; i < parts; i++) {
            std::vector<ContractTempTxMsgReq> part;
...
int ContractDispatcher::DistributionContractTx(std::multimap<std::string, msgInfo>& distribution)
{
    ...
    std::vector<std::vector<ContractTempTxMsgReq>> dependentData = GetDependentData();
    ...
    std::vector<std::vector<ContractTempTxMsgReq>> groupedData = GroupDependentData(dependentData);
    ...
    for(const auto & txMsgContainer : groupedData)
    {
        ...
        std::vector<ContractTempTxMsgReq> txMsgData;
