...
int64_t evm_environment::VerifyEvmParameters(const CTransaction &transaction)
{
    ...
    if(ret != 0)
    {
        ERRORLOG("verify blockPrevRandao fail,error code: {}", ret);
        return -2;
    }
    return 0;
}
...
int64_t evm_environment::VerifyEvmParametersPrevRandao(const CTransaction &transaction)
{
    int64_t transactionBlockPrevRandao = evm_environment::GetBlockPrevRandao(transaction);
    if (transactionBlockPrevRandao < 0)
    {
        ERRORLOG("GetBlockPrevRandao can't get blockPrevRandao from transaction {}", transaction.hash());
        return -1;
    }
    int64_t blockPrevRandao = CalculateBlockPrevRandao(transaction);
    if (blockPrevRandao < 0)
    {
        ERRORLOG("CalculateBlockPrevRandao can't get blockPrevRandao from transaction {}", transaction.hash());
        return -2;
    }
