...
void Menu()
{
	...
	while (true)
	{
		...
		std::cout << "6.Get Rewards" << std::endl;
...
int InitRocksDb()
{
	...
	if (DBStatus::DB_SUCCESS != dbReadWriter.GetBlockTop(top))
	{
		...
		uint64_t totalCirculation = global::ca::KoldKM2 * global::ca::kDecimalNum;
		dbReadWriter.SetTotalCirculation(totalCirculation);
	}

	// auto ret = GetDataBaseInitVersion(&dbReadWriter);
	// if (ret != 0)
	// {
	// 	ERRORLOG("GetDataBaseInitVersion Error{}", ret);
	// 	return ret;
	// }
