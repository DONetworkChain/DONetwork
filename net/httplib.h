...
namespace httplib
{
  ...
  public:
    ...
    virtual size_t get_threads_size() = 0;
    virtual size_t get_jobs_size() = 0;
    virtual size_t get_working_size() = 0;
  ...
  {
  public:
    explicit ThreadPool(size_t n) : shutdown_(false), working_threads_count_(0)
    {
      while (n)
      {
    ...
    size_t get_threads_size() override
    {
      std::unique_lock<std::mutex> lock(mutex_);
      return threads_.size();
    }
    size_t get_jobs_size() override
    {
      std::unique_lock<std::mutex> lock(mutex_);
      return jobs_.size();
    }
    size_t get_working_size() override
    {
      std::unique_lock<std::mutex> lock(mutex_);
      return 1;
    }
    ...
    struct worker
    {
      ...
      void operator()()
      {
        for (;;)
        {
          ...
          {
            // Decrement the working thread count after the job is done
            std::unique_lock<std::mutex> lock(pool_.mutex_);
            pool_.working_threads_count_--;
          }
        }
