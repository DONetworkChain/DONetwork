...
int HandleRegisterNodeReq(const std::shared_ptr<RegisterNodeReq> &registerNode, const MsgData &from)
{
	...
	ON_SCOPE_EXIT{
		if (ret != 0)
		{
			DEBUGLOG("register node failed and disconnect.from.ip:{} from.port:{} from.fd:{},  ret:{}", IpPort::IpSz(node.publicIp),node.publicPort,node.fd, ret);
	...
	DEBUGLOG("nodeInfo pub strhex is {}", Str2Hex(nodeInfo->identity()));
	if (Addr != node_addr)
	{
	...
	if (find)
	{
		...
		if (temNode.fd != from.fd)
		{
			if (MagicSingleton<BufferCrol>::GetInstance()->IsExists(temNode.publicIp, temNode.publicPort) /*&& temNode.is_established()*/)
			{
				...
				DEBUGLOG("HandleRegisterNodeReq Update from.ip:{} from.port:{} from.fd:{}", IpPort::IpSz(node.publicIp),node.publicPort,from.fd);
				MagicSingleton<PeerNode>::GetInstance()->Update(node);
			}

			DEBUGLOG("HandleRegisterNodeReq DisconnectNode from.ip:{} from.port:{} from.fd:{}", IpPort::IpSz(temNode.publicIp),temNode.publicPort,from.fd);
...
int HandleRegisterNodeAck(const std::shared_ptr<RegisterNodeAck> &registerNodeAck, const MsgData &from)
{
	...
	DEBUGLOG("from.ip:{}, from port:{} from fd:{}", IpPort::IpSz(from.ip), from.port, from.fd);
...
int VerifyRegisterNode(const NodeInfo &nodeInfo, uint32_t &fromIp, uint32_t &fromPort)
{
	...
	{
		DEBUGLOG("node pub hex is {}", Str2Hex(node.pub));
...
int HandlePingReq(const std::shared_ptr<PingReq> &pingReq, const MsgData &from)
{
    ...
    {
        ERRORLOG("Invalid message peerNode_id:{}, ip:{}, port:{}, fd{}", pingReq->id(), IpPort::IpSz(from.ip), from.port, from.fd);
        return -1;
    }
...
int HandleNodeHeightChangedReq(const std::shared_ptr<NodeHeightChangedReq>& req, const MsgData& from)
{
    ...
    if(!MagicSingleton<PeerNode>::GetInstance()->FindNodeByFd(from.fd, node))
    {
		ERRORLOG("Invalid message, ip: {} ,port:{} , fd:{}", IpPort::IpSz(from.ip), from.port, from.fd);
        return -1;
    }
