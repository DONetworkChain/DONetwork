...
bool PeerNode::add(const Node& _node)
{
	...
	if(MagicSingleton<Config>::GetInstance()->_Verify(_node) != 0)
	{
		return false;
	}
...
{
	Node node;
    if(!MagicSingleton<PeerNode>::GetInstance()->find_node_by_fd(fd, node))
    {
        ERRORLOG("Invalid message ");
        return false;
    }
    if(peerId != node.base58address)
    {
        ERRORLOG("Invalid message peerNode_id:{}, node.base58address:{}", peerId, node.base58address);
        return false;
    }
	return true;
}