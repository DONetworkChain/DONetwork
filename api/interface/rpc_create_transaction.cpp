...
std::map<int, std::string> unstakeErrorList = {
	{-1, "Get all stake address failed"},
	{-2, "The account number has not staked assets"},
	{-3, "Get stake utxo from address failed"},
	{-4, "The utxo to be de staked is not in the staked utxo"},
	{-5, "The staked utxo is not more than 30 days"},
	{-6, "Stake tx not found"},
	{-7, "Failed to parse transaction body"},
	{-8, "Stake value is zero"},
};
...
void ReplaceCreatUnstakeTransaction(const std::string& fromAddr, const std::string& utxoHash, bool isFindUtxo, const std::string& encodedInfo, txAck* ack)
{
	...
	if(ret != 0)
	{
		auto it = unstakeErrorList.find(ret);
		std::string errorMessage;
		if(it != unstakeErrorList.end()){
			errorMessage = it->second;
		}else{
			errorMessage = "IsQualifiedToDisinvest error";
		}
...
std::map<int, std::string> investmentErrorList = {
	{-1, "The investor have already invested in a node"},
	{-2, "The investment amount is less than minimum amount"},
	{-3, "The invested account is not pledged"},
	{-4, "Get invest addrs by node failed"},
	{-5, "The account number to be invested have been invested by 999 people"},
	{-6, "GetBonusAddrInvestUtxosByBonusAddr failed"},
	{-7, "GetTransactionByHash failed"},
	{-8, "Failed to parse transaction body"},
	{-9, "The total amount invested in a single node will be more than Maximum investment amount"}
};
								...
								const std::string& toAddr,uint64_t investAmount, int32_t investType, bool isFindUtxo, const std::string& encodedInfo, txAck* ack)
{
	...
	if(ret != 0)
	{	
		auto it = investmentErrorList.find(ret);
		std::string errorMessage;
		if(it != investmentErrorList.end()){
			errorMessage = it->second;
		}else{
			errorMessage = "CheckInvestQualification error";
		}

        ackT->code = ret - 100;
        ackT->message = errorMessage;
        return;
	}	
...
std::map<int, std::string> disinvestmentErrorList = {
	{-1, "GetBonusAddrByInvestAddr failed"},
	{-2, "The account has not invested assets to node"},
	{-3, "GetBonusAddrInvestUtxosByBonusAddr failed"},
	{-4, "The utxo to divest is not in the utxos that have been invested"},
	{-5, "The invested utxo is not more than 1 day"},
	{-6, "Invest tx not found"},
	{-7, "Failed to parse transaction body"},
	{-8, "The node to be divested is not invested"},
	{-9, "The invested value is zero"},
};
                                    ...
                                    const std::string& toAddr, const std::string& utxoHash, bool isFindUtxo, const std::string& encodedInfo, txAck* ack)
{
	...
	ret = IsQualifiedToDisinvest(fromAddr, toAddr, utxoHash, investedAmount);
	if(ret != 0)
	{
		auto it = disinvestmentErrorList.find(ret);
		std::string errorMessage;
		if(it != disinvestmentErrorList.end()){
			errorMessage = it->second;
		}else{
			errorMessage = "IsQualifiedToDisinvest error";
		}
...
std::map<int, std::string> bounsErrorList = {
	{-1, "The bouns time is not within the prescribed limits"},
	{-2, "Incorrect time"},
	{-3, "GetBonusUtxoByPeriod fail"},
	{-4, "GetTransactionByHash fail"},
	{-5, "Failed to parse transaction body"},
	{-6, "GetBonusUtxoByPeriod fail"},
	{-7, "VerifyBonusAddr fail"},
	{-8, "The bouns address is not pledged"},
};
...
void ReplaceCreateBonusTransaction(const std::string& addr, bool isFindUtxo, const std::string& encodedInfo, txAck* ack)
{
	...
	if(ret != 0)
	{
		auto it = bounsErrorList.find(ret);
		std::string errorMessage;
		if(it != bounsErrorList.end()){
			errorMessage = it->second;
		}else{
			errorMessage = "CheckBonusQualification error";
		}
