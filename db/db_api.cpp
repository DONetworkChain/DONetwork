...
const std::string KM2Flag = "M2flag_";
const std::string KComputeHashFlag = "ComputeHashFlag_";
...
DBStatus DBReader::GetKM2FlagTime(uint64_t &time)
{
    std::string value;
    auto ret = ReadData(KM2Flag, value);
    if (DBStatus::DB_SUCCESS == ret)
    {
        time = std::stoull(value);
    }
    return ret;
}

DBStatus DBReader::GetKComputeHashFlag(uint64_t height)
{
    std::string value;
    auto ret = ReadData(KComputeHashFlag, value);
    if (DBStatus::DB_SUCCESS == ret)
    {
        height = std::stoull(value);
    }
    return ret;
}
...
DBStatus DBReadWriter::SetKM2FlagTime(uint64_t time)
{
    return WriteData(KM2Flag, std::to_string(time));
}

DBStatus DBReadWriter::SetKComputeHashFlag(uint64_t height)
{
    return WriteData(KComputeHashFlag, std::to_string(height));
}
