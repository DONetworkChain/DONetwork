...
public:
    ...
    DBStatus GetKM2FlagTime(uint64_t &time);

    DBStatus GetKComputeHashFlag(uint64_t height);
    /**
     * @brief
     * 
     * @param       totalBurn:
...
public:
    ...
    DBStatus SetKM2FlagTime(uint64_t height);

    DBStatus SetKComputeHashFlag(uint64_t height);
    /**
     * @brief       Setting up the utxo account of encumbered assets
     *
